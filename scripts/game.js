function resetGameStatus() {
  ...
  currentRound = 1;
  gameIsOver = false;
  gameOverElement.firstElementChild.innerHTML = `Hurray!!! You have won the game
            <span id="winner__name">PLAYER NAME !</span>`;
...
function startNewGame() {
  if (players[0].name === '' || players[1].name === '') {
    alert('Paila naam rakh ani matrai khalna pauxas. Dhanyabaad ');
    return;
  }
  resetGameStatus();
...
function selectgameField(event) {
  ...
  if (gameData[selectedRow][selectedColumn]) {
    alert('please select on another field');
    return;
  }
